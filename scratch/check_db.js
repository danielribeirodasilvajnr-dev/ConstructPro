...
async function run() {
  ...
  const grimaldo = projects.find(p => p.name.includes('GRIMALDO') || p.name.includes('TOLAINI'));
  if (!grimaldo) {
    console.log('Grimaldo project not found');
    return;
  }

  console.log('Project ID for Grimaldo:', grimaldo.id);

  // Find budget items
  const { data: budgetItems, error: bErr } = await supabase.from('budget_items').select('*').eq('project_id', grimaldo.id);
  if (bErr) {
  ...
  const { data: financialItems, error: fErr } = await supabase.from('financial_items').select('*').eq('project_id', grimaldo.id);
  if (fErr) {
