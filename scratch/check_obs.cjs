...
async function run() {
  ...
  console.log('--- Current Budget Sub-Items in Database ---');
  subItems.forEach(s => {
  ...
  console.log('\n--- Financial Items and observations ---');
  financialItems.forEach(f => {
    if (f.observations && f.observations.includes('budget_sub_item_linked_id:')) {
      console.log(`- Financial ID: ${f.id} | Desc: ${f.description} | Obs: ${f.observations}`);
    }
  });
}
