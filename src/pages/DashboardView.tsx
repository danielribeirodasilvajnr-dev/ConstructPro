...
export function DashboardView() {
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto pb-24">
      {dashError && (
        <div className="bg-red-500 text-on-surface p-4 font-mono text-sm mb-4">
      ...
      <div className="space-y-20">
        {dashboardProjects.length === 0 ? (
          <div className="glass-panel p-24 text-center rounded-[32px] border-outline text-on-surface-variant font-display uppercase tracking-widest animate-pulse">
          ...
          dashboardProjects.map((project, i) => (
            <div key={i} className="relative group">
              ...
              <div className="relative bg-surface/60 backdrop-blur-2xl rounded-[32px] overflow-hidden border border-outline shadow-2xl transition-all duration-500 group-hover:translate-y-[-4px]">
                {/* Project Header - Glassmorphic Bento Header */}
                <div className="p-10 pb-8 border-b border-outline flex flex-col lg:flex-row lg:items-center justify-between gap-8 bg-gradient-to-br from-white/5 to-transparent">
                  <div>
                    ...
                    <h4 className="text-3xl font-display font-bold text-on-surface tracking-wide uppercase">{project.name}</h4>
                    <div className="flex flex-wrap gap-6 mt-4">
                      <div className="bg-surface-container-low px-4 py-2 rounded-xl border border-outline flex items-center gap-3 group/info">
                      ...
                      <div className="bg-surface-container-low px-4 py-2 rounded-xl border border-outline flex items-center gap-3">
                        <p className="text-on-surface-variant text-[11px] font-bold uppercase tracking-wider"><span className="text-on-surface/40 mr-2">ÁREA:</span> {project.area || '0'} m²</p>
                      </div>
                      <div className="bg-surface-container-low px-4 py-2 rounded-xl border border-outline flex items-center gap-3">
                        <p className="text-on-surface-variant text-[11px] font-bold uppercase tracking-wider"><span className="text-on-surface/40 mr-2">ENTREGA:</span> {project.deadline || 'N/D'}</p>
                      </div>
                    </div>
                  </div>
                  <div className={cn(
                ...
                <div className="p-10">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-12">
                    {[
                      { label: 'VALOR DO CONTRATO', value: formatCurrency(project.ordained), icon: Wallet, color: 'text-on-surface' },
                      ...
                      { label: 'REALIZADO (OBRAS)', value: formatCurrency(project.budgetSpent || 0), icon: Wallet, color: 'text-on-surface' },
                      ...
                      <div key={idx} className={cn(
                        "p-8 rounded-[28px] border border-outline backdrop-blur-xl transition-all duration-500 hover:border-primary/40 group/card relative overflow-hidden",
                        ...
                        {stat.label === 'SALDO EM CAIXA' && (
                          <p className="text-[9px] text-on-surface-variant mt-2 font-bold uppercase tracking-widest">Aporte - Gastos</p>
                        )}
                        {stat.label === 'REALIZADO (OBRAS)' && (
                          <p className="text-[9px] text-on-surface-variant mt-2 font-bold uppercase tracking-widest">{project.physicalProgress}% do orçamento</p>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-4 space-y-12">
                      {/* Progress Sections */}
                      <div className="space-y-8 bg-surface-container-low/40 p-10 rounded-[32px] border border-outline backdrop-blur-xl">
                        <h5 className="text-[10px] font-display font-bold text-on-surface uppercase tracking-[4px] mb-2">Monitoramento de Fluxo</h5>
                        <div>
                          ...
                          <div className="h-2 w-full bg-surface-container-low rounded-full overflow-hidden p-[1px]">
                            <motion.div 
                              initial={{ width: 0 }}
                        ...
                        <div>
                          <div className="flex justify-between items-end mb-4">
                            ...
                            <span className="text-sm font-display font-bold text-on-surface">{project.physicalProgress}%</span>
                          </div>
                          <div className="h-2 w-full bg-surface-container-low rounded-full overflow-hidden p-[1px]">
                            <motion.div 
                              initial={{ width: 0 }}
                      ...
                      {/* Timeline Bento Card */}
                      <div className="bg-surface-container-low/20 rounded-[32px] border border-outline p-10 relative overflow-hidden group/timeline backdrop-blur-xl">
                        ...
                        <h5 className="text-[10px] font-display font-bold text-on-surface uppercase tracking-[4px] mb-8 flex items-center gap-2">
                          ...
                          {project.logs.slice(0, 3).map((log: any, j: number) => (
                            <div key={j} className="flex gap-6 items-start group/log">
                              <div className="relative">
                                ...
                                {j !== 2 && <div className="absolute left-[3.5px] top-5 w-[1px] h-12 bg-surface-container-low" />}
                              </div>
                              <div>
                                <p className="text-[10px] font-display font-bold text-primary tracking-widest uppercase mb-1.5">{log.date}</p>
                                <p className="text-xs text-on-surface-variant font-medium leading-relaxed group-hover:text-on-surface transition-colors">{log.desc}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                      {/* Evolution Chart */}
                      <div className="md:col-span-2 bg-background/30 border border-outline rounded-[40px] p-8 flex flex-col hover:border-primary/20 transition-all duration-500">
                        <div className="flex items-center justify-between mb-10">
                          <h5 className="text-[10px] font-display font-bold text-on-surface uppercase tracking-[4px]">ANÁLISE DE EVOLUÇÃO TEMPORAL</h5>
                        ...
                        <div className="flex-1 min-h-[300px]">
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={project.evolutionData}>
                              ...
                              <Tooltip 
                                contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-outline)', borderRadius: '16px', backdropFilter: 'blur(12px)' }}
                                itemStyle={{ color: 'var(--color-primary)', fontSize: '12px', fontWeight: 'bold' }}
                                labelStyle={{ color: 'var(--color-on-surface-variant)', marginBottom: '4px', fontSize: '10px' }}
                              />
                              <Line type="monotone" dataKey="real" stroke="var(--color-primary)" strokeWidth={4} dot={{ r: 4, fill: 'var(--color-primary)', strokeWidth: 2, stroke: 'var(--color-surface)' }} activeDot={{ r: 6, strokeWidth: 0 }} />
                      ...
                      {/* Cost Bar Chart */}
                      <div className="md:col-span-2 bg-background/30 border border-outline rounded-[40px] p-8 flex flex-col hover:border-primary/20 transition-all duration-500">
                        <h5 className="text-[10px] font-display font-bold text-on-surface uppercase tracking-[4px] mb-10">DISTRIBUIÇÃO DE CUSTOS OPERACIONAIS</h5>
                        <div className="flex-1 min-h-[300px]">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={project.costData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                              ...
                              <Tooltip 
                                ...
                                contentStyle={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-outline)', borderRadius: '16px' }}
                              />
                              <Bar dataKey="previsto" fill="var(--color-outline)" radius={[12, 12, 0, 0]} />
                              <Bar dataKey="realizado" fill="var(--color-primary)" radius={[12, 12, 0, 0]} />
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                      </div>
